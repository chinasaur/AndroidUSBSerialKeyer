 ...
 */
#ifndef USBKEYEROBOEJAVA_OSCILLATOR_H
#define USBKEYEROBOEJAVA_OSCILLATOR_H

#include <cstdint>
#include <atomic>
//#include <math.h>  // Deprecated; can replace with cmath?
#include <cmath>
#include <memory>
...
private:
    ...
    std::atomic<float> mPhaseIncrement { 0.f };
    float mFrequency = kDefaultFrequency;
    int32_t mSampleRate = kDefaultSampleRate;

    void updatePhaseIncrement() {
        mPhaseIncrement.store((kTwoPi * mFrequency) / static_cast<float>(mSampleRate));
    };
};

#endif //USBKEYEROBOEJAVA_OSCILLATOR_H